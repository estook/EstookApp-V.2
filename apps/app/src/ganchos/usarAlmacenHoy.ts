...
import type { AlmacenHoy } from '../almacen/contrato.ts';

/**
 * Lo que hay que atender del almacén, pedido **una vez** para todo el Panel.
 *
 ...
 *
 * Sin la app de Almacén en el acceso, o sin estar dentro de un local, esto se
 ...
 */
export function usarAlmacenHoy(): UseQueryResult<AlmacenHoy> {
  const { cliente, permisos, yo } = usarSesion();
  const loTiene = puedeVer(permisos, 'app.almacen');
