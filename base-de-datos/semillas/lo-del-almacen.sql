...
-- **Por eso se llama `lo-del-almacen.sql` y no `almacen.sql`** (25-sep): al
-- renombrar Inventario a Almacén, `almacen.sql` pasaba a cargarse la primera,
-- antes de que hubiera un solo local, y la prueba de migraciones lo cazó.
--
...
-- ejemplo se quedaban sin una sola categoria. Lo encontro la prueba del
-- almacén, y no la migracion: la migracion siembra los locales que ya
...
-- un unico dueno, que es `packages/dominio/src/almacen.ts`, y se siembra
-- desde el servidor (`sembrarElAlmacen`). Escribir esas cuentas otra vez en
