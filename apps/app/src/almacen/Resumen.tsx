...
import { NOMBRE_DEL_ESTADO, comoSePide, plural } from '@estook/dominio';
...
/** El acento de Almacén, que pinta el icono de cada tarjeta de aquí. */
const ACENTO = 'var(--color-app-almacen)';
...
/**
 * Almacén · Resumen (M6; «Hoy» hasta la entrega V).
 *
...
export function Resumen({ alAbrirProducto }: { readonly alAbrirProducto: (id: string) => void }) {
  ...
  const consulta = useQuery({
    queryKey: ['almacen_hoy'],
    queryFn: async (): Promise<AlmacenHoy> => {
      const respuesta = await cliente.consultar<AlmacenHoy>('almacen_hoy');
  ...
  if (consulta.isError) {
    return (
      <Aviso tono="mal" titulo="No he podido leer tu almacén">
  ...
  return (
    <div className="flex flex-col gap-e5">
      ...
      {hoy.cuantosProductos === 0 ? (
        ...
        <>
          {/*
            «Cómo va» · las cifras de Almacén con su flecha, **arriba del todo**.
            ...
            arriba del todo en Almacén, ya que es lo que más se ve». Es una fila de
            ...
            la misma cuenta que `almacen_hoy` (una prueba contra la base lo vigila).
          */}
          <CifrasDeLaApp app="almacen" />
          ...
          <Mosaico>
            ...
            */}
            ...
            {/*
              Lo congelado, aparte (repaso del 25-sep, 0049): no caduca, se queda
              viejo. Sale una semana antes de cumplir lo que aguanta congelado.
            */}
            {hoy.congelados.length > 0 && (
              <Tarjeta
                titulo="Lleva tiempo congelado"
                icono={<IconoCongelado size={18} />}
                acento={ACENTO}
                cuantos={hoy.congelados.length}
              >
                <ul className="flex flex-col gap-e1">
                  {hoy.congelados.map((lote) => (
                    <li key={lote.loteId} className="flex items-center gap-e2">
                      <button
                        type="button"
                        onClick={() => {
                      ...
                      >
                        ...
                        >
                          {cuandoCumpleCongelado(lote.dias, lote.meses)}
                          {lote.cuanto === null
                            ? ''
            ...
            {puedeVer(permisos, 'app.almacen') && <ComprasDeHoyEnHoy />}
 ...
 * botón van los tres pasos que hacen útil Almacén, en una línea cada uno: quien
  ...
  return (
    <Tarjeta>
      <EstadoVacio
        ...
        {...(alta === null
          ? { sinAccionPorque: 'El género lo da de alta quien lleva el almacén del local.' }
...
/** Los tres pasos que hacen útil Almacén, en el orden en que se dan. */
...
function LineaDeAtencion({
  ...
  const puedePedir = puedeEditar(permisos, 'app.almacen');
        ...
        <div className="flex flex-wrap items-center justify-between gap-e2">
          ...
          {producto.proveedorId !== null && puedePedir && (
            <Boton
              tono="principal"
              onClick={() => {
                navegar(`/almacen/compras/pedidos?pedir=${producto.proveedorId ?? ''}`);
              }}
            >
...
function SinPrecio({
  ...
  readonly productos: AlmacenHoy['sinPrecio'];
  readonly alAbrirProducto: (id: string) => void;
}) {
  const [todos, setTodos] = useState(false);
    ...
    <Tarjeta
      ...
      accion={
        <EnlaceDeTarjeta
          ...
          onClick={() => {
            navegar('/almacen/compras/pedidos');
          }}
        />
      }
    >
...
/** «Cumple 3 meses congelado en 5 días», «Cumplió 3 meses congelado hace 2 días». */
function cuandoCumpleCongelado(dias: number, meses: number): string {
  const tiempo = plural(meses, 'mes', 'meses');
  if (dias < 0) {
    return `Cumplió ${tiempo} congelado hace ${plural(Math.abs(dias), 'día', 'días')}`;
  }
  if (dias === 0) return `Cumple ${tiempo} congelado hoy`;
  if (dias === 1) return `Cumple ${tiempo} congelado mañana`;
  return `Cumple ${tiempo} congelado en ${dias} días`;
}
