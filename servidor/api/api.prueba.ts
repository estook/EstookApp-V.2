...
describe('el versionado, con compatibilidad hacia atras', () => {
  ...
  it('la API responde con un error en cristiano, no con un numero pelado', async () => {
    const { app } = api();
    const respuesta = await app.request('/api/v99/consultas/mis_locales');
...
/**
 * ── Esto no es una prueba de adorno ──────────────────────────────────────────
 *
 * El primer despliegue que llego a completarse contestaba `404` a todo. La
 * funcion estaba viva y Hono respondiendo; lo que pasaba es que Supabase sirve
 * las funciones en `/functions/v1/<nombre>/...` y **le pasa a la funcion la ruta
 * con su propio nombre delante**, asi que llegaba `/api/salud` a una API que
 * solo conocia `/salud`.
 *
 * Es de los errores mas desconcertantes que hay: parece que no esta desplegada
 * cuando lo esta y funciona. Asi que queda fijado en una prueba, en los dos
 * sentidos: que la raiz es la que Supabase manda, y que sin ella no hay nada.
 */
describe('la raiz que impone Supabase', () => {
  it('todo cuelga de /api, que es lo que llega desplegado', async () => {
    const { app } = api();
    expect((await app.request('/api/salud')).status).toBe(200);
  });

  it('y sin ella no responde nadie, que es como se descubrio', async () => {
    const { app } = api();
    expect((await app.request('/salud')).status).toBe(404);
  });
});
...
describe('lo que no existe', () => {
  it('una consulta que no esta en el catalogo', async () => {
    const { app } = api();
    const cuerpo = await cuerpoDe(await app.request('/api/v1/consultas/inventada'));
    expect(cuerpo.error?.codigo).toBe('no_existe');
  });

  it('un comando que no esta en el catalogo', async () => {
    const { app } = api();
    const respuesta = await app.request('/api/v1/comandos/inventado', {
      method: 'POST',
...
describe('todo comando necesita su clave', () => {
  it('sin la cabecera de idempotencia no se ejecuta nada', async () => {
    const { app } = api();
    const respuesta = await app.request('/api/v1/comandos/cambiar_mi_idioma', {
      method: 'POST',
...
describe('sin decir quien pregunta no se ve nada', () => {
  it('una consulta sin persona devuelve «la sesion ha caducado»', async () => {
    const { app } = api();
    const cuerpo = await cuerpoDe(await app.request('/api/v1/consultas/mis_locales'));
...
describe('cada respuesta lleva su hilo', () => {
  it('devuelve la correlacion que se le mando', async () => {
    ...
    const respuesta = await app.request('/api/v1/consultas/mis_locales', {
