 ...
 *
 * El tercero —reconstruir el libro— se comprueba en `almacen.prueba.ts`
...
/** Marcos cocina en Bar Centro: Almacén entera y **ningún importe**. */
...
/**
 * Va a un destino de Almacén, con su vista si la tiene.
 *
 * `irAAlmacen(page, 'productos')` cae en `/almacen/productos/todo`, que es
 ...
 */
async function irAAlmacen(page: Page, destino: string, vista?: string) {
  const camino = vista === undefined ? destino : `${destino}/${vista}`;
  await abrirSinQueSeCaiga(page, `${APP}#/almacen/${camino}`);
}
...
test('se da de alta un producto en menos de treinta segundos', async ({ page }) => {
  ...
  await irAAlmacen(page, 'productos');
 ...
 * con `almacen_hoy`, o sea con esa caché. En M22 eso es lo que se le manda al
 * modelo.
 */
test('al cambiar de local, la pantalla no se queda con el género del anterior', async ({
  ...
  await irA(page, '#/almacen/productos');
 ...
 *
 * `almacen_hoy` acaba en un bloque que busca los lotes que caducan pronto:
 *
 ...
 *
 * No lo cazó nada porque **ninguna prueba llamaba a `almacen_hoy`**: ni las de
...
test('«Resumen» contesta, en vez de caerse con un 500', async ({ request }) => {
  ...
  expect(hoy.estado, 'almacen_hoy no contesta 200').toBe(200);
...
test('el libro se pinta por días, con quién apuntó cada línea', async ({ page }) => {
  await entrar(page, ROSA);
  await irAAlmacen(page, 'movimientos', 'todo');
...
test('las cuatro vistas de Productos se abren, y ninguna se queda muda', async ({ page }) => {
  ...
  for (const vista of ['todo', 'bajo-minimo', 'sin-precio', 'desactivados']) {
    await irAAlmacen(page, 'productos', vista);
...
test('un producto se da de alta con nombre, unidad y precio, sin hacer cuentas', async ({
  ...
  await entrar(page, ROSA);
  await irAAlmacen(page, 'productos', 'todo');
  ...
  await hoja.getByLabel('Cuánto hay ahora', { exact: true }).fill('12');
...
test('la ficha no vende ingredientes, deja medir el aprovechamiento y se puede deshacer', async ({
  ...
  await abrirSinQueSeCaiga(
    page,
    `${APP}#/almacen/productos/todo?producto=${creado.datos?.productoId ?? ''}`,
  );
 ...
 *
 * Almacén era un solo montón: un cocinero buscando harina pasaba por las
...
test('el recuento cambia lo que hay y dice cuánto bailaba', async ({ page, request }) => {
  ...
  await page.getByRole('button', { name: /Cerrar el inventario/ }).click();
  await expect(page.getByText('Inventario cerrado')).toBeVisible({ timeout: 15_000 });
