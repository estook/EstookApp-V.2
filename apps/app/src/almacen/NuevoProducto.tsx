...
export function NuevoProducto({
  ...
  /**
   * El mínimo, **desde el alta** (repaso del 25-sep). Solo salía en la ficha, y un
   * producto nuevo se quedaba sin aviso hasta que alguien entraba a buscarlo.
   * Richi: «mejor que te lo pida desde el principio, para que no se pase».
   */
   ...
   * Almacén ha sido la cámara hasta hoy: quien no se fije acierta.
   */
  ...
  const minimoNumero = minimo.trim() === '' ? null : Number(minimo.replace(',', '.'));
  const minimoVale = minimoNumero === null || (Number.isFinite(minimoNumero) && minimoNumero >= 0);
  const listo = nombre.trim() !== '' && hayVale && minimoVale && !guardando;
  // Lo congelado no caduca: se queda viejo (0049). Con «Está congelado», la fecha
  // de caducidad ni se pregunta ni se manda.
  const conCaducidad = !(hayAlgo && congelado);
  /**
   * En qué se cuenta, que se toca en la propia unidad del campo: «kg ▾» (repaso
   * del 25-sep). Cambiarla cambia en qué se cuenta el producto, que en el alta, sin
   * nada apuntado todavía, se puede.
   */
  const opcionesDeUnidad = unidadesQueSeEligen(como.modo);
  const unidadQueSeElige: UnidadQueSeElige | undefined =
    opcionesDeUnidad.length === 0
      ? undefined
      : {
          valor: presentacion.unidadDeUso,
          opciones: opcionesDeUnidad,
          alElegir: (unidad) => {
            const elegida = opcionesDeUnidad.find((o) => o.valor === unidad);
            if (elegida !== undefined) setComo({ ...como, unidad: elegida.valor });
          },
        };
        ...
        {enElFormulario && (
          <>
            ...
            */}
            ...
            {/* 4 · Cuánto hay y el mínimo, uno al lado del otro y en la misma unidad */}
            <div className="grid gap-e3 sm:grid-cols-2">
              <Campo
                etiqueta="Cuánto hay ahora"
                tipo="numero"
                value={cuantoHay}
                {...(unidadQueSeElige === undefined
                  ? { detras: enQueSeCuenta }
                  : { unidad: unidadQueSeElige })}
                ayuda="Lo que tienes ahora. Entra con su precio, para que la cámara valga desde hoy."
                onChange={(e) => {
              ...
              <Campo
                ...
                {...(unidadQueSeElige === undefined
                  ? { detras: enQueSeCuenta }
                  : { unidad: unidadQueSeElige })}
                ayuda="Por debajo de esto te aviso en «Hoy»."
                {...(minimoVale ? {} : { error: 'Pon una cantidad, o déjalo en blanco.' })}
                onChange={(e) => {
            ...
            {hayAlgo && (
              <Interruptor
                ...
                ayuda="Sale en «Congelados». No caduca: te aviso cuando lleve tiempo."
                puesto={congelado}
