...
function elLocal(contexto: Contexto): string {
  ...
  if (!localId) {
    throw new FalloDeAplicacion('faltan_datos', {
      porque: 'Hay que estar dentro de un local para ver su almacén. Elige uno primero.',
    });
  }
  return localId;
}
...
interface FilaDeProducto {
  ...
  congelado_aguanta_meses: number;
...
async function leerProductos(
  ...
  const filas = await contexto.sql<FilaDeProducto[]>`
    ...
    select el.total_filas,
           ...
           p.congelado_aguanta_meses::int as congelado_aguanta_meses,
...
function componer(
  ...
  const producto: ProductoEnLista = {
    ...
    congeladoAguantaMeses: fila.congelado_aguanta_meses,
  };
 ...
 * usan la lista de Productos y el Resumen de Almacén, y por eso vive aquí una
...
export interface LoteEnFicha {
   ...
   * Nulos si no está congelado. Lo congelado avisa por esto, no por su caducidad.
   */
  ...
  async ejecutar(contexto, entrada) {
    ...
    >`
      select l.id, l.codigo,
             ...
             to_char(l.recibido_el, 'YYYY-MM-DD') as recibido_el,
             (l.caduca_el - ${hoy}::date)::int as dias,
             to_char(l.congelado_el, 'YYYY-MM-DD') as congelado_el,
             to_char(l.congelado_el + make_interval(months => p.congelado_aguanta_meses::int),
                     'YYYY-MM-DD') as cumple_el,
             ((l.congelado_el + make_interval(months => p.congelado_aguanta_meses::int))::date
               - ${hoy}::date)::int as dias_para_cumplir,
             l.cantidad::text as cantidad
        from estook.lote l
        join estook.producto p on p.id = l.producto_id
       where l.producto_id = ${entrada.producto_id}
         and l.retirado_en is null
       order by l.caduca_el nulls last, l.recibido_el desc
       limit 30
    `;
    ...
    return {
      ...
      lotes: lotes.map((l) => ({
        ...
        cumpleCongeladoEl: l.cumple_el,
        diasParaCumplir: l.dias_para_cumplir,
...
export interface SalidaAlmacenHoy {
  ...
  /**
   * Lo congelado que cumple, o está a punto de cumplir, lo que aguanta congelado
   * (repaso del 25-sep, 0049). Lo congelado no sale en `caducan`: no caduca, se
   * queda viejo.
   */
  readonly congelados: readonly {
    ...
    /** El día que cumple lo que aguanta congelado. */
    readonly cumpleEl: string;
    /** Los días que le faltan. Cero o menos: ya lo ha cumplido. */
    readonly dias: number;
    readonly meses: number;
    /** Cuánto hay en ese lote, si se dijo al congelar. */
...
export const almacenHoy = consulta<Record<string, never>, SalidaAlmacenHoy>({
  nombre: 'almacen_hoy',
  ...
  async ejecutar(contexto) {
    ...
    >`
      select l.id as lote_id, l.producto_id, p.nombre as producto, l.codigo as lote,
       ...
       where l.local_id = ${localId}
         ...
         and l.retirado_en is null
         -- Lo congelado no caduca: se queda viejo, y tiene su propia lista
         -- (repaso del 25-sep, 0049).
         and l.congelado_el is null
    ...
    // Lo congelado, por lo que lleva en el congelador contra lo que aguanta ese
    // producto congelado (0049): desde una semana antes de cumplirlo.
    const congelados = await contexto.sql<
      {
        lote_id: string;
        producto_id: string;
        producto: string;
    ...
    >`
      select lote_id, producto_id, producto, congelado_el, to_char(cumple, 'YYYY-MM-DD') as cumple_el,
             (cumple - ${hoy}::date)::int as dias, meses, cuanto, unidad_de_uso
        from (
          select l.id as lote_id, l.producto_id, p.nombre as producto,
                 to_char(l.congelado_el, 'YYYY-MM-DD') as congelado_el,
                 (l.congelado_el + make_interval(months => p.congelado_aguanta_meses::int))::date as cumple,
                 p.congelado_aguanta_meses::int as meses,
                 l.cantidad::text as cuanto,
                 p.unidad_de_uso::text as unidad_de_uso
            from estook.lote l
            join estook.producto p on p.id = l.producto_id
           where l.local_id = ${localId}
             and not p.es_ejemplo
             and p.activo
             and l.congelado_el is not null
             and l.retirado_en is null
        ) c
       where cumple <= ${hoy}::date + ${AVISO_DE_LO_CONGELADO_EN_DIAS}::int
       order by cumple
       limit 50
    `;
    ...
    return {
      ...
      congelados: congelados.map((c) => ({
        loteId: c.lote_id,
        productoId: c.producto_id,
        producto: c.producto,
        congeladoEl: c.congelado_el,
        cumpleEl: c.cumple_el,
        dias: c.dias,
        meses: c.meses,
        cuanto: c.cuanto === null ? null : Number(c.cuanto),
        unidadDeUso: c.unidad_de_uso,
      })),
      sinPrecio,
